...
package com.atakmap.android.usingfragments.plugin;
...
import com.atakmap.android.usingfragments.UsingFragmentsMapComponent;
...
public class UsingFragmentsLifecycle implements Lifecycle {

    private final Context pluginContext;
    private final Collection<MapComponent> overlays;
    private MapView mapView;

    private final static String TAG = "UsingFragmentsLifecycle";

    public UsingFragmentsLifecycle(Context ctx) {
    ...
    public void onCreate(final Activity arg0,
        ...
        UsingFragmentsLifecycle.this.overlays
                .add(new UsingFragmentsMapComponent());

        // create components
        Iterator<MapComponent> iter = UsingFragmentsLifecycle.this.overlays
                .iterator();
        MapComponent c;
        while (iter.hasNext()) {
            c = iter.next();
            try {
                c.onCreate(UsingFragmentsLifecycle.this.pluginContext,
                        arg0.getIntent(),
                        UsingFragmentsLifecycle.this.mapView);
            } catch (Exception e) {
                Log.w(TAG,
